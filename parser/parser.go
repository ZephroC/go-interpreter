...
//TODO: peek token can be mutated so this code layout between function calls is bad
func (p *Parser) peekError(tt token.TokenType) {
	err := fmt.Errorf("expected next token to be of type: %s, got %s instead", tt, p.peekToken.Type)
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	i, err := strconv.ParseInt(p.curToken.Literal, 0, 64)
	if err != nil {
		p.errorLog = append(p.errorLog, fmt.Errorf("could not parse %s as an integer", p.curToken.Literal))
		return nil
	}
...
func (p *Parser) noPrefixParseFnError(t token.TokenType) {
	p.errorLog = append(p.errorLog, fmt.Errorf("no prefix parse function for %s found", t))
}
